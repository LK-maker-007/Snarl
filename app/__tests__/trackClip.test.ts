import {ModelRunner, trackClip} from '../src/ml/trackClip';
...
function blankFrame(): Uint8Array {
  return new Uint8Array([0, 0, 0, 255, 0, 0, 0, 255]); // two RGBA pixels
}

describe('trackClip', () => {
  it('decodes the centre frame of each window and leaves the ends null', async () => {
    const frames = [blankFrame(), blankFrame(), blankFrame(), blankFrame()];
    const track = await trackClip(centrePeakRunner(), frames, 2, 1, 0.5);
  ...
  it('throws when the model output size does not match the frame dimensions', async () => {
    ...
    const frames = [blankFrame(), blankFrame(), blankFrame()];
    await expect(trackClip(wrongSize, frames, 2, 1)).rejects.toThrow();
  });
});
